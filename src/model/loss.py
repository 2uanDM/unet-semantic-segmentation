...
def calc_dice_score(input, target, eps=1e-6):
    input = mask2rgb(input.argsmax(dim=1))
    
    target = mask2rgb(target)
    
    dims = (2, 3)
    intersect = torch.sum(input * target, dims)
    sum_car = torch.sum(input + target, dims)
    
    dice_score = (2. * intersect + eps) / (sum_car + eps)
